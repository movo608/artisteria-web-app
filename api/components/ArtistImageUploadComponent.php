 ...
 */
class ArtistImageUploadComponent extends Component
{
	/**
	...
	{
		$time = time();
		if ($model->image1->saveAs('../' . $this::$UPLOAD_DIR . $time . 'artist-image-1' . '.' . $model->image1->extension)
			&& 
			$model->image2->saveAs('../' . $this::$UPLOAD_DIR . $time . 'artist-image-2' . '.' . $model->image1->extension)) {
			$model->image1 = $time . 'artist-image-1' . '.' . $model->image1->extension;
			$model->image2 = $time . 'artist-image-2' . '.' . $model->image2->extension;
			return true;
		} else {
			return false;
		}
